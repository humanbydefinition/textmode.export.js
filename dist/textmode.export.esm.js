...
class ge {
  ...
  $generateSVG(e, t = {}) {
    const r = new We(), a = new He();
    console.log("Textmodifier:", e), console.log("Layers:", e.layers), console.log("Draw Framebuffer:", e.layers.base);
    const i = r.$extractSVGCellData(r.$extractFramebufferData(e.layers.base.drawFramebuffer), e.grid), n = a.$generateSVGContent(i, e.grid, e.font, this._applyDefaultOptions(t));
    ...
    return await new Promise((l, p) => {
      ...
      const x = () => {
        ...
        if (u >= c) h(o);
          ...
          const _ = t.scale, v = Math.max(1, Math.round(e.width * _)), g = Math.max(1, Math.round(e.height * _));
          ...
          const C = this._ctx;
          C.imageSmoothingEnabled = !1, C.clearRect(0, 0, v, g), C.drawImage(e, 0, 0, e.width, e.height, 0, 0, v, g);
...
var Pe = Object.defineProperty, Qe = (s) => Pe(s, "__esModule", { value: !0 }), Xe = (s, e) => () => (e || s((e = { exports: {} }).exports, e), e.exports), Ye = (s, e) => {
  ...
  function c(d, u) {
    let [m, x, h] = d, [f, _, v] = u, g = e(m, x, h) - e(f, _, v), C = t(m, x, h) - t(f, _, v), S = r(m, x, h) - r(f, _, v), E = i(d) - i(u);
...
function et(s, e, t, r, a, i, n, c) {
  ...
  i.fill(0), c.fill(0), L();
  let o = 0, d = 0, u = p + 1, m = u, x = !1, h = m, f = D(h), _ = 1 << u - 1, v = _ + 1, g = _ + 2, C = 0, S = t[0], E = 0;
...
function at(s, e, t) {
  ...
  for (h = 0; h < S; ) {
    for (var E; ; ) {
      ...
      for (E.mtm == u ? A = m[1] = m[m[0]--] : (ye(o, A), E.tm = h), C = o[A].err, v = 1; (g = v + v) <= m[0] && (g < m[0] && o[m[g]].err > o[m[g + 1]].err && g++, !(C <= o[_ = m[g]].err)); v = g) m[v] = _;
      m[v] = A;
    }
  ...
  for (h = 0; ; ++O) {
    ...
    if (it(D, F) || D.push(F), (h = o[h].fw) == 0) break;
  }
  return D;
}
function it(s, e) {
...
function lt(s = {}) {
  ...
  return { reset() {
    ...
    if (t ? c || (C = !0, l(), c = !0) : C = !!u.first, o = Math.max(0, Math.floor(o)), d = Math.max(0, Math.floor(d)), C) {
      ...
      ct(r, o, d, f, v), _e(r, f), _ >= 0 && ut(r, _);
    }
    let S = Math.round(h / 10);
    dt(r, g, S, m, x);
    let E = !!f && !C;
    pt(r, o, d, E ? f : null), E && _e(r, f), ht(r, p, o, d, v, a, i, n);
  } };
  function l() {
    Re(r, "GIF89a");
  }
...
class mt {
  ...
  async $saveGIF(e = {}) {
    ...
    try {
      ...
      i.forEach((o, d) => {
        const { width: u, height: m, imageData: x, delayMs: h } = o, f = new Uint32Array(x.data.buffer.slice(0)), _ = at(f, 256, {}), v = ot(f, _);
...
var Be, Ie, Se, Fe = { exports: {} };
function vt() {
  return Be || (Be = 1, s = Fe, (function() {
    let e = function(t) {
      return function(r) {
            ...
            if (c) return new Promise(function(x, h) {
              p(u.data).then(function(f) {
                let _ = 0, v = Buffer.from(f.buffer), g = function(C, S, E) {
                  _ += S, _ >= E.length ? x() : t.write(c, E, _, E.length - _, u.offset + _, g);
                };
                t.write(c, v, 0, v.length, u.offset, g);
              });
            });
...
function xt() {
  return Se ? Ie : (Se = 1, Ie = gt()(bt(), vt()(null)));
}
var yt = xt();
const _t = ft(yt);
class wt {
...
const X = "textmode-export-number-input", It = `${X}__field`, St = `${X}__controls`, Ft = `${X}__control`, Ue = `${X}__display`, te = `${Ue}--visible`, Lt = (s) => {
  s.dispatchEvent(new Event("input", { bubbles: !0 })), s.dispatchEvent(new Event("change", { bubbles: !0 }));
}, Me = (s) => {
  const e = document.createElement("button");
  return e.type = "button", e.className = Ft, e.textContent = s > 0 ? "▲" : "▼", e;
};
class G extends T {
  props;
  input;
  ...
  render() {
    ...
    const t = document.createElement("div");
    return t.className = St, this.incrementButton = Me(1), this.decrementButton = Me(-1), t.appendChild(this.incrementButton), t.appendChild(this.decrementButton), e.appendChild(this.input), e.appendChild(this.display), e.appendChild(t), this.bindStepControls(), this.bindHoldBehavior(), this.bindInputListeners(), this.observeDisabledState(e), this.updateDisplay(), e;
  }
  get inputElement() {
    return this.input;
  }
...
class ue extends Y {
  ...
  handleProgress(e) {
    e.state === "recording" && e.totalFrames ? this.status.setMessage(`recording ${e.frameIndex ?? 0}/${e.totalFrames}`, "active") : e.state === "completed" ? this.status.setMessage("GIF saved successfully", "active") : e.state === "error" && this.status.setMessage(e.message ?? "failed to export GIF", "alert");
  }
  applyDefaults() {
