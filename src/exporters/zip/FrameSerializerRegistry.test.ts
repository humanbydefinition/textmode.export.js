...
describe('ZIP frame serializer registry', () => {
	...
	it('forces raster format and strips untrusted filename and format fields', async () => {
		...
		expect(
			createZIPFrameSerializerRegistry({} as never).png.getDimensions({ canvas, frameOptions: { scale: 2 } })
		).toEqual({
			width: 600,
			height: 300,
		});
	...
	it('rejects a browser fallback blob whose MIME type does not match the requested frame format', async () => {
		vi.spyOn(ImageExporter.prototype, '$toImageBlob').mockResolvedValue(
			new Blob([new Uint8Array([137, 80, 78, 71])], { type: 'image/png' })
		);

		await expect(
			createZIPFrameSerializerRegistry({} as never).webp.serialize({
				canvas: document.createElement('canvas'),
				frameOptions: {},
				createdAt,
			})
		).rejects.toMatchObject({ code: 'ZIP_EXPORT_ENCODING_UNSUPPORTED' });
	});
	...
	it('forwards complete text-format controls and encodes UTF-8 without a BOM', async () => {
		...
		const jsonString = vi.spyOn(JSONExporter.prototype, '$generateJSONString').mockReturnValue('{"frame":1}');
		...
		const svg = await registry.svg.serialize({
			canvas: document.createElement('canvas'),
			frameOptions: {},
			createdAt,
		});
		const json = await registry.json.serialize({
			canvas: document.createElement('canvas'),
			frameOptions: {},
			createdAt,
		});
		...
		expect(jsonString).toHaveBeenCalledWith(textmodifier, {}, createdAt);
